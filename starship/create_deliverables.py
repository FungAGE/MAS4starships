...
This script creates a fasta, genbank, gff, faa, tbl, sqn, and annotation table for the selected starship.
...
def parse_args():
    arg_parser = argparse.ArgumentParser(
        description='This script creates a fasta, genbank, gff, faa, tbl, '
                    'sqn, and annotation table for the selected starship.')
    ...
    arg_parser.add_argument('starship_name', type=str, help='Name of the starship we would like to create deliverables for')
...
def write_fsa(genome, out_file, starship_name):
    '''
    Create a fsa (fasta) file representing the complete starship starship.
    Note: The 'fsa' extension is required by the tbl2asn tool
    :param starship: Biopython Seq object containing full starship genome
    :param out_file: file path to write to
    :param starship_name: Name of starship
    '''
    record = SeqRecord(genome, id=starship_name, description='')
    SeqIO.write(record, out_file, 'fasta')


def write_faa(df, out_file, starship_name, only_annotated=False):
    '''
    ...
    for i, row in df.iterrows():
        if row['Feature Type'] == 'CDS':
            ...
            prot_id = '%s_%s' % (starship_name, str(prot_num).rjust(len(str(num_cds)), '0'))
...
def write_gff(df, out_file, starship_name, starship_length):
    '''
    Create a gff which represents all features
    :param df: Dataframe with all annotation data
    :param out_file: file path to write to
    :param starship_name: Name of starship
    :param starship_length: Length of entire starship genome
    '''
    with open(out_file, 'w') as fh:
        ...
        for i, row in df.iterrows():
            ...
            # Write gff line
            line = '%s\t.\t%s\t%s\t%s\t.\t%s\t.\t%s\n' % (starship_name, row['Feature Type'],
                                                          start, stop, strand, attributes)
            fh.write(line)


def write_tbl(df, out_file, starship_name, starship_length):
    """
    ...
    with open(out_file, 'w') as fh:
        ...
        for i, row in df.iterrows():
            ...
            feature_header = db_standard_to_tbl_text(row['Feature Start'], row['Feature End'],
                                                     row['Strand'], starship_length, row['Feature Type'])
...
def collect_merge_values_for_docx(df, starship_name, genome):
    merge_values = {}

    merge_values['starship_name'] = starship_name
    merge_values['starship_size'] = '{:,}'.format(len(genome))
    merge_values['gc_content'] = '{:.2%}'.format(GC(genome) / 100)

    try:
        merge_values['host_name'] = starship_name[4:starship_name.find('_Phi_')]
    except:
...
def write_docx(df, out_file, starship_name, genome):
    """
    Create a partially completed polished report for the starship. Some fields still need to be filled out manually.
    """
    from mailmerge import MailMerge

    merge_values = collect_merge_values_for_docx(df, starship_name, genome)

    # Create document
    template_path = os.path.join(os.path.dirname(os.path.realpath(__file__)), 'polished_starship_report_template.docx')
    document = MailMerge(template_path)
    document.merge(**merge_values)
    document.write(out_file)


def shorten_starship_name(starship_name):
    host_gs = starship_name[4] + starship_name[6]
    phi_num = starship_name[starship_name.find('Phi_') + 4:starship_name.find('Phi_') + 7]
    shortened = host_gs + phi_num

    if starship_name.count('_', starship_name.find('Phi_')) > 1:
        shortened += starship_name[-1]
...
def main(args):
    ...
    starship_obj = JoinedShips.objects.get(starshipID=args.starship_name)
    ...
    if 'xlsx' in args.deliverables or len(args.deliverables) == 0:
        ...
        excel_path = os.path.join(args.output_folder, '%s.xlsx' % args.starship_name)
    ...
    if 'fsa' in args.deliverables or len(args.deliverables) == 0:
        # Write starship genome to fsa
        fsa_path = os.path.join(args.output_folder, '%s.fsa' % args.starship_name)
        if test_save(fsa_path):
            write_fsa(genome, fsa_path, args.starship_name)

    if 'faa' in args.deliverables or len(args.deliverables) == 0:
        # Save protein sequences to faa
        faa_path = os.path.join(args.output_folder, '%s.faa' % args.starship_name)
        if test_save(faa_path):
            ...
            if hasattr(args, 'only_annotated') and args.only_annotated == True:
                write_faa(df, faa_path, args.starship_name, args.only_annotated)
            else:
                write_faa(df, faa_path, args.starship_name)

    if 'gff' in args.deliverables or len(args.deliverables) == 0:
        # Write features to gff
        gff_path = os.path.join(args.output_folder, '%s.gff' % args.starship_name)
        if test_save(gff_path):
            write_gff(df, gff_path, args.starship_name, len(genome))

    if 'tbl' in args.deliverables or len(args.deliverables) == 0:
        # Write features to tbl
        tbl_path = os.path.join(args.output_folder, '%s.tbl' % args.starship_name)
        if test_save(tbl_path):
            write_tbl(df, tbl_path, args.starship_name, len(genome))
    ...
    if 'sqn' in args.deliverables or len(args.deliverables) == 0:
        ...
        #     '-i', fsa_path,
        #     '-j', "[genome=%s][gcode=11]" % args.starship_name,
        #     '-n', args.starship_name,
        #     '-Z', os.path.join(args.output_folder, '%s_tbl2asn_discrepancy_report.txt' % args.starship_name)
        # ]
        #
        ...
        cmd = 'LD_PRELOAD=/home/daemon/miniconda/envs/mas/lib/python3.8/site-packages/libfaketime/vendor/libfaketime/src/libfaketime.so.1 FAKETIME="2019-01-01 00:00:01" ' \
              f'tbl2asn -i "{fsa_path}" -j [genome="{args.starship_name}"][gcode=11] -n "{args.starship_name}" -Z "{os.path.join(args.output_folder, "%s_tbl2asn_discrepancy_report.txt" % args.starship_name)}" -V b'
        subprocess.run(cmd, check=True, shell=True)

        # Fix locus line
        if 'gbf' in args.deliverables or len(args.deliverables) == 0:
            gbf_path = os.path.join(args.output_folder, '%s.gbf' % args.starship_name)
            first_line = True
            for line in fileinput.input(files=[gbf_path], inplace=True):
                if first_line:
                    line = line.rstrip()
                    after_name_loc = line.find(args.starship_name) + len(args.starship_name)
...
#         docx_path = os.path.join(args.output_folder, '%s_Polished.docx' % args.starship_name)
#         if test_save(docx_path):
#             write_docx(df, docx_path, args.starship_name, genome)
