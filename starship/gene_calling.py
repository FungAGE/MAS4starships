...
def run_glimmer(fasta_path, starship_name, output_destination, minimum_gene_len=90):
    '''
    ...
    # Run long-orfs
    long_orf_file = open(os.path.join(output_destination, '%s.glimmer.longorfs' % starship_name), 'w')
    subprocess.run(['long-orfs', '-n', '-t', '1.1', fasta_path, '-'], stdout=long_orf_file, check=True)

    # Run extract
    extract_file = open(os.path.join(output_destination, '%s.glimmer.train' % starship_name), 'w+')
    subprocess.run(['extract', '-t', fasta_path, long_orf_file.name], stdout=extract_file, check=True)
    extract_file.seek(0)

    # Run build icm
    icm_file_path = os.path.join(output_destination, '%s.glimmer.icm' % starship_name)
    ...
    subprocess.run(['glimmer3', '-o50', '-g{}'.format(minimum_gene_len), '-t30', fasta_path, icm_file_path,
                    os.path.join(output_destination, '%s.glimmer' % starship_name)], check=True)

    long_orf_file.close()
    extract_file.close()

    return os.path.join(output_destination, '%s.glimmer.predict' % starship_name)


def run_trnascan_se(fasta_path, starship_name, output_destination):
    '''
    Run tRNAscan-SE and return output file
    '''
    print('*** RUNNING GLIMMER ***')

    out_file_path = os.path.join(output_destination, '%s.trnascan_se.results.txt' % starship_name)
